...
class ReadStoreInfoController(
    ...
    @GetMapping("/store")
    fun readStoreInfo(
        @Valid @ParameterObject readStoreRequest: ReadStoreRequest,
        @Parameter(hidden = true) @LoginUser user: User
    ): ApiResponse<ReadStoreResponse> {
        val readStoreResponse = storeService.readStore(readStoreRequest, user)
